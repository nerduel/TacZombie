...
object JsonHelper {
  ...
  case class Error(cmd : String = "error", message : String)
  ...
  class String2ErrorJsonHelper(str : String) {
    def toErrorJson : String = {
      import ErrorJsonProtocol._
      Error(message = str).toJson.toString
    }
  }
  ...
  implicit def string2ErrorJsonWrapper(str : String) =
    new String2ErrorJsonHelper(str)
}
